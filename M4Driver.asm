...
;
; IX-Register
; call 	FF06_IX__Get_M4_Socket_Response_Address	; IN: C=ROM-Number OUT:  HL=Socket response adress    (ld		hl,($FF06)	; get Socket response address)
;
; IY-Register
; call  FF02_IY__Get_M4_Buffer_Response_Address; IN: C=ROM-Number OUT HL=Adresse			ld		hl,($FF02)	; get response buffer address
...
; m4 commands used
M4_C_NETSOCKET		equ $4331		; data[0] = domain, data[1] = type, data[2] = protocol. Return data[0] = socket number or $FF (error). 							Only TCP protocol for now.
M4_C_NETCONNECT		equ $4332		; data[0] = socket, data[1..4] = ip, data[5-6] = port. Return data[0] = 0 (OK) or ERROR $FF.
M4_C_NETCLOSE		equ $4333
M4_C_NETSEND		equ $4334
M4_C_NETRECV		equ $4335		;data[0] = socket, data[1..2] = receive size (don't flood buffer, max. $800).
									; Return data[0] = 0. data[1..2] = actual received size. data[3...] = received data. Look at sockinfo for status.

M4_C_NETHOSTIP		equ $4336	; data[0..] = hostname string\0. Return data[0] = 1 Lookup in progress. Any other = error. Look in M4rom sockinfo for IP and status
;Responses are read from rom by mapping M4 rom.
;At offset $FF00, there is a link table for things that may move as firmware gets updates:
;$FF00		.dw	$109			; rom version
;$FF02		.dw	rom_response		; $800+some byte buffer for responses from commands
;$FF04		.dw	rom_config		; Internal config, that is only reset by power cycle. This buffer can be written with <1 byte size> .dw C_CONFIG <offset> <data>
;$FF06		.dw	sock_info			; socket structure for NETAPI, works like read only hardware registers
;$FF08		.dw	helper_functions	; useful functions, to have executed in rom.

;Socket info structure (ptr read from $FF06), there is a total of 4 sockets for use and "socket 0" reserved C_NETHOSTIP. User sockets are 1-4. offset:
...
; Return data[0] = socket number or $FF (error). Only TCP protocol for now.
			...
			call    FF02_IY__Get_M4_Buffer_Response_Address	; OUT HL=Adresse			ld		hl,($FF02)	; get response buffer address
...
FF02_IY__Get_M4_Buffer_Response_Address
			...
			ld hl,$FF02
			push bc
			call Read16BitFromROM	; IN: C=ROM-Number, HL=Adress   OUT: HL=Value	;			ld		hl,($FF02)	; get response buffer address
			pop bc
			pop af
			ret

FF06_IX__Get_M4_Socket_Response_Address	; OUT:  HL=Socket response adress    (ld		hl,($FF06)	; get Socket response address)
			push af
			ld hl,$FF06
			push bc
			call Read16BitFromROM	; IN: C=ROM-Number, HL=Adress   OUT: HL=Value	;			ld		hl,($FF06)	; get Socket response address
			pop bc
			pop af
			ret
...
Read16BitFromROM	; IN: C=ROM-Number, HL=Adress   OUT: HL=Value	;			ld		hl,($FF02)	; get response buffer address
			push ix
			push de
			push bc
...
M4_CMD_NET_LOOKUP_IP_wait
			call 	FF06_IX__Get_M4_Socket_Response_Address	; OUT:  HL=Socket response adress    (ld		hl,($FF06)	; get Socket response address)
...
; ------------------------------------------------------------------------------
;C_NETRECV        	$4335		Implemented v1.0.9.	data[0] = socket, data[1..2] = receive size (don't flood buffer, max. $800).
;							Return data[0] = 0. data[1..2] = actual received size. data[3...] = received data. Look at sockinfo for status.
